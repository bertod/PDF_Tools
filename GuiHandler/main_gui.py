...
class Gui(QMainWindow):
    ...
    def __init__(self):
        ...
        self.removeButton.clicked.connect(partial(self.clear_upload, self.fileList))
    ...
    @pyqtSlot()
    def clear_upload(self, listWidget, uploaded_files_list=None):
        uploaded_files_list = []
        for index in range(listWidget.count()):
            file_to_remove = listWidget.item(index).text()
            filename = file_to_remove.split("/")[-1]
            uploaded_files_list.append(filename)
            # uploaded_files_list.append(listWidget.item(index))
        listWidget.clear()
        print(uploaded_files_list)
        self.helper_pdf.flush_temp(files_to_remove=uploaded_files_list)
        return []
